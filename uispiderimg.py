...
def getAndSaveImg(imgUrl):  
    ...
    if( len(imgUrl)!= 0 ):  
        ...
        print('/nimgUrl= ', imgUrl)
        print('/nlocalPath= ', localPath)
        try:
...
#    print(urls)
    for url in urls:
      bFlag= redishelper.hexists('imghandled', url)
      if bFlag:
        continue
      redishelper.hset('imghandled', url, url)
      opener=makeMyOpener()
      try:
        response=opener.open(url, timeout=1000)
      except urllib2.HTTPError:
      ...
      for links in soup.findAll("img"):
        print("获得本页所有图片", links.get('src'))
        if (links.get('src')==None):
          continue
        getAndSaveImg(links.get('src'))
